...
def test_load_pod_file_exists(mock_open, _mock_is_symlink, _mock_exists) -> None:
    """Test load pod file."""
    workdir = pathlib.Path(get_workdir())
...
@mock.patch("pathlib.Path.is_symlink", return_value=True)
@mock.patch("pathlib.Path.readlink", return_value=pathlib.Path("deleted"))
@mock.patch(
    "pathlib.Path.open",
    new_callable=mock.mock_open,
    read_data='{"name": "pod1", "status": "running"}',
)
def test_load_pod_file_deleted(
    mock_open, _mock_readlink, _mock_is_symlink, _mock_exists
) -> None:
    """Test load pod file."""
    workdir = pathlib.Path(get_workdir())
    pod_data = api._load_pod_file(workdir, "pod1")
...
@mock.patch("hostfactory.api._fetch_pod_symlink_status", return_value="")
...
def test_get_request_status(
    ...
    mock_fetch_pod_symlink_status.assert_called_once()
    mock_fetch_pod_symlink_status.assert_called_with(pathlib.Path(workdir), "pod1")
